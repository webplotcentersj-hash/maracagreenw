    ...
    >
       ...
       >
         {statusBars.map((bar, index) => (
           <motion.div
             key={bar.id}
             className={cn("h-0.5 rounded-full", bar.id === "1" ? "bg-emerald-500/80" : bar.id === "2" ? "bg-emerald-500/50" : "bg-emerald-500/20")}
             style={{ 
               opacity: bar.opacity,
             ...
             return (
               ...
               >
                 ...
                 >
                   ...
                   />
                   <div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-slate-950 to-transparent"></div>
                   {card.gradientColors && (
                     <div className={`absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t ${card.gradientColors[0]} ${card.gradientColors[1]} to-transparent`}></div>
                   )}
                   ...
                   >
                     <div className="text-[10px] font-bold font-mono px-2 py-0.5 bg-slate-950/80 border border-slate-800/40 rounded uppercase text-emerald-400 w-fit mb-2">
                       {card.category}
                     </div>
                     <div className="text-[11px] font-light text-slate-300">
                 ...
                 >
                   <motion.h3 
                     ...
                     className="font-bold text-white text-base leading-snug line-clamp-3 group-hover:text-emerald-400 transition-colors"
                   >
                     {card.title}
                   </motion.h3>
                   
                   <div className="flex items-center justify-between text-xs text-slate-500 mt-4 pt-4 border-t border-slate-900/60 font-mono">
                     <span>{card.subcategory}</span>
                     <span className="text-emerald-400 font-bold group-hover:translate-x-1 transition-transform">Leer Artículo &rarr;</span>
                   </div>
                 </motion.div>
               </motion.article>
             );
           ...
           {selectedCard && (
             <>
               ...
               <motion.div
                 ...
                 className="fixed inset-4 md:inset-8 lg:inset-20 bg-[#070b10] border border-slate-800/80 rounded-2xl overflow-hidden z-50 shadow-2xl max-w-4xl mx-auto"
               >
                 {/* Close Button */}
                 <motion.button
                   className="absolute top-4 right-4 w-9 h-9 bg-slate-950/80 hover:bg-slate-900 text-white rounded-full flex items-center justify-center z-50 border border-slate-800/40 backdrop-blur-sm"
                 ...
                 <div className="h-full overflow-y-auto custom-scrollbar">
                   ...
                   >
                     ...
                     />
                     <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-[#070b10] to-transparent"></div>
                     ...
                     <div className="absolute bottom-6 left-6 text-white">
                       <span className="text-xs font-bold font-mono px-2.5 py-1 bg-emerald-950/60 border border-emerald-500/20 text-emerald-400 rounded-md uppercase tracking-wider block mb-2 w-fit">
                         {selectedCard.category} • {selectedCard.subcategory}
                       </span>
                       <div className="text-xs opacity-75 font-light text-slate-300">{selectedCard.timeAgo} • {selectedCard.location}</div>
                     </div>
                   </motion.div>
                   ...
                   >
                     <motion.h1 
                       ...
                       className="text-xl md:text-3xl font-extrabold text-white mb-6 leading-snug"
                     >
                       {selectedCard.title}
                     </motion.h1>
                     
                     <motion.div 
                       className="prose prose-invert prose-emerald max-w-none text-slate-300 text-sm md:text-base leading-relaxed space-y-4 font-light"
